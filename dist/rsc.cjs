...
var vfileMatter = require('vfile-matter')
...
var _chalk$1 = _interopRequireWildcard$1(chalk$1.exports, true)
...
var _chalk = _interopRequireWildcard(chalk.exports, true)
...
async function serialize(
  ...
  const vfile$1 = new vfile.VFile(source)
  // makes frontmatter available via vfile.data.matter
  if (parseFrontmatter) {
    vfileMatter.matter(vfile$1, { strip: true })
  }
  let compiledMdx
  try {
    compiledMdx = await mdx.compile(vfile$1, getCompileOptions(mdxOptions, rsc))
  } catch (error) {
    throw createFormattedMDXError(error, String(vfile$1))
  }
  let compiledSource = String(compiledMdx)
  return {
    compiledSource,
    frontmatter: vfile$1.data.matter ?? {},
    scope: scope,
  }
}
...
async function compileMDX({ source, options, components = {} }) {
  ...
  const fullScope = Object.assign(
    {
      opts: jsxRuntime_cjs.jsxRuntime,
    },
    { frontmatter },
    scope
  )
  ...
  return {
    content: React__default['default'].createElement(Content, {
